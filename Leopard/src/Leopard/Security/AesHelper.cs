...
using Leopard.Domain.Exceptions;
using Leopard.Extensions;

namespace Leopard.Security
{
    /// <summary>
    /// AEA加密解密辅助类
    /// </summary>
    public class AesHelper
    {
        ...
        {
            Check.NotNull(decodeBytes, nameof(decodeBytes));
            
            using (Aes aes = Aes.Create())
            {
                if (aes == null)
                {
                    throw new LeopardException("AES加密时获取加密实例失败");
                }
        ...
        {
            Check.NotNull(encodeBytes, nameof(encodeBytes));
        ...
        {
            Check.FileExists(sourceFile, nameof(sourceFile));
